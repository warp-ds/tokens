	...
	uniqueBrands.forEach((brand) => {
		...
		try {
			...
			const androidConfig = {
				source: [
					colorTokenFilePath,
					semanticTokenFilePath,
					componentTokenFilePath,
				],
				platforms: {
					android: {
...
function generateXmlIdsForAndroid() {
	...
	const colorTokenFilePath = path.join(tokensPath, `finn-light`, "colors.json");
	const semanticTokenFilePath = path.join(
		tokensPath,
		`finn-light`,
		"semantic.json",
	...
	try {
		...
		console.error(`Error building android xml color ids : ${error.message}`);
	}
}
