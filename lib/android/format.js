...
import { toCamelCase, getGeneratedDate } from "./helpers.js";

// Get override for the given component name, from the dictionary.
function getOverride(dictionary, componentName, dataviz = false) {
  return dictionary.allProperties
    .filter((token) => token.path[0].startsWith("semantic")) // Exclude primitive colors, only keep semantic tokens
    .map((token) => tokenToOverride(token, componentName, dataviz))
    .filter(Boolean)
    .join("\n");
}

// Convert token to override, with the given component name.
function tokenToOverride(token, componentName, dataviz = false) {
  let name = toCamelCase(token.path.slice(dataviz ? 2 : 1).join("-")); // Convert to camelCase, removing first two parts
...
// Get brand name, with the given string to remove.
function getBrandName(file, colorString = "Colors.kt") {
  return file.destination.split("/").pop().replace(colorString, ""); // Extract brand name from the file path
}
...
function generateDatavizColorTokens(file, dictionary, isDark = false) {
  ...
  const brandName = file.destination.split("/").pop().replace("Colors.kt", ""); // Extract brand name from the file path

  const brandNameLowercase =
    brandName.charAt(0).toLowerCase() + brandName.slice(1).replace("Dark", "");
  ...
  const semanticColorBlockBackground = getOverride(
    dictionary,
    "background",
    true
  );
...
function generateColorTokens(file, dictionary, isDark = false) {
  ...
  const semanticColorBlockSurface = getOverride(dictionary, "surface");
  const semanticColorBlockBackground = getOverride(dictionary, "background");
  const semanticColorBlockBorder = getOverride(dictionary, "border");
  const semanticColorBlockIcon = getOverride(dictionary, "icon");
  const semanticColorBlockText = getOverride(dictionary, "text");
...
<resources>`;
    ...
    const idEntriesComponents = dictionary.allProperties
      ...
      .map((token) => {
        ...
        const name = nameParts
          .map((part) =>
            part.replace(/ /g, "_").replace(/-/g, "_").toLowerCase()
          )
          .join("_");
